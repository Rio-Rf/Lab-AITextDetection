...
from binoculars.detector import BINOCULARS_ACCURACY_THRESHOLD as THRESHOLD # 閾値をaccuracyで固定
# from binoculars.detector import BINOCULARS_FPR_THRESHOLD as THRESHOLD # 閾値をlow-fprで固定
...
def main(args):
    ...
    #breakpoint()
    args.experiment_path = f"results/test/{args.job_name}"
    ...
    # Interpolate the TPR at FPR = 0.2%, this is a fixed point in roc curve
    tpr_at_fpr_0_2 = np.interp(0.2 / 100, fpr, tpr)

    # Save experiment
    save_experiment(args, score_df, fpr, tpr, f1_score, roc_auc, tpr_at_fpr_0_2)
