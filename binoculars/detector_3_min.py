...
BINOCULARS_ACCURACY_THRESHOLD = 0.9134
...
class Binoculars(object):
    ...
    def compute_score(self, input_text: Union[list[str], str]) -> Union[float, list[float]]:
        ...
        # スコアをリストにまとめる
        binoculars_score_1 = binoculars_score_1.tolist()
        binoculars_score_2 = binoculars_score_2.tolist()
        binoculars_score_3 = binoculars_score_3.tolist()

        binoculars_scores = []
        min_indices = []

        # 各スコアの最大値とそのインデックスを選択
        for i in range(len(binoculars_score_1)):
            scores = [binoculars_score_1[i], binoculars_score_2[i], binoculars_score_3[i]]
            min_value = min(scores)
            min_index = scores.index(min_value) + 1  # 1, 2, 3のどのスコアかを示す
            binoculars_scores.append(min_value)
            min_indices.append(min_index)
        
        # 最大値のスコアとどのスコアから選ばれたかを表示
        #print("Binoculars scores (minimum values):", binoculars_scores)
        print("Min score chosen from (1, 2, or 3):", min_indices)
